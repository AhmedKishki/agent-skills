...
Create `{project}-full-export.zip` only when the user explicitly requests a **full export** or a handoff package. Never infer the request from a pause, approval milestone, migration, or conversation change.
...
Preserve each relative path and file's extracted bytes. Include active canonical files, source originals, conversions, current combined sources, and non-active migration/reconciliation inputs when they have tracker rows. Add no untracked file, prior tracker, superseded/history file, or full-export ZIP.
