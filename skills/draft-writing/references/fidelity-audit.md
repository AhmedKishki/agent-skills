...
- every assistant white box synthesis—and legacy assistant synthesis marked white-box compatible—used only original Human inputs and closed-list operations, disclosed every non-COPY change, remained terminal Mixed at 100% Human wording, passed its target/metatheory tests, and retained its complete final white box record;
- no assistant synthesis underlying the draft crossed an unreported substantive gap or used a black-box transformation;
- every AI gap passage retains named search coverage, unsupported/non-synthesis status, source limits, exact predraft/arc approval, a matching approved report exception, and controlled draft use;
...
User selection never cures a fidelity failure. Identify whether it belongs to [predraft](predraft.md), the [article arc](article-arc.md), or the [predraft report](predraft-report.md). State every invalidated dependency, suggest the relevant module, and wait for user direction. Repaired inputs must regain approval before predraft review and then drafting become available again. Never correct substantive draft prose in place.
