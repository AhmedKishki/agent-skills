...
Record item codes only (A1, B2-MT); copy no wording. The index is a downstream retrieval aid, never evidence, synthesis input, article wording, or authority to suppress or alter an independently mapped source item. Membership never substitutes for role, relevance, or fit decisions. Assistant-selected labels use exact Human words but remain AI-produced.
    ...
    - **Process prose:** AI
    - **Index content:** AI · Human wording: 100%

    ## Themes
    | Theme | Keywords | Human basis |
    |---|---|---|
    | theme name | keyword, keyword | exact thesis/user/source-map references |

    ## Keywords
    | Keyword | Items | Human basis |
    |---|---|---|
    | keyword phrase | A1, A2, B2-MT | exact thesis/user/source-map references |

The declaration covers assistant-selected rows; stamp only exact user-assigned exceptions locally. Infer only retrieval labels and assignments; never infer source meaning, role, relevance, relation, or metatheory fit. Ask when two materially different mappings would change retrieval. Omit the Themes section when no theme meets its rule. Before a rescan or synthesis, query a relevant theme and then its keywords; query an ungrouped keyword directly. Verify every candidate against its source map. Save and track per [file-output-standard.md](file-output-standard.md).
