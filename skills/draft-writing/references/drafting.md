...
The current draft thesis, an approved unit of the draft plan, its assigned draft material, relevant user material and source maps, and direct user prose or revisions.
...
`{project}-draft.md` owns current reader-facing article prose. Draft material remains the owner of underlying claims, bases and developing synthesis; the draft plan remains the owner of structure.

## Procedure

1. Work from the current plan, one complete user-approved unit at a time.
2. Present the unit's purpose, assigned draft material and unresolved needs before composing.
...
6. Save approved prose in the live draft and update draft progress.
...
<Reader-facing article prose in the current draft-plan order.>
```

The draft contains only reader-facing prose. Keep current working questions in draft progress and underlying records in draft material. The exact section structure and prose remain flexible under the approved draft plan.

## Citation verification

Identify each support-dependent claim and reopen its source. Check identity, locator, date, scope, modality and qualification. Propose footnotes or other citations in the style set by the draft thesis. If support fails, report the defect and return to draft material; do not silently qualify prose. Create a separate citation work file only when the user requests it or the scale of unresolved citation work gives it a distinct current purpose.

## Completion

Every unit is approved, the prose conforms to the current draft thesis and plan, support-dependent claims are verified, no prose was added during assembly, and the user approves the current complete article.
