...
6. Identify every approved downstream passage affected by a changed thesis constraint, arc claim, argument, source basis or user original; mark each one `Invalidated`.
...
The diff contains no unapproved substantive change, all references resolve and every affected downstream passage has the correct state.

## Blocking condition

Stop before editing when intent, origin, correspondence or migration scope is ambiguous.
