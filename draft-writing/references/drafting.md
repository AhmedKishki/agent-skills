...
The draft is a candidate until the user explicitly approves it. A requested change reopens its canonical owner and the predraft-report loop; never patch substantive draft prose directly. Correct only a mechanical execution error that departs from already approved dependencies, then record it in the changelog.
